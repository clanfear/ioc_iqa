...
date: "3 Dec 2025<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%d %b %Y'))`"
output:
...
.text-80[
```{r}
tidy(full_model) |> mutate(across(-term, ~round(., 3))) # round everything
```
]
...
.text-80[
```{r}
anova(neighb_model, full_model) |> tidy() # You can tidy anova() output
```
]
...
tidy(full_model) |> mutate(across(-term, ~round(., 3))) |> 
  filter(str_detect(term, "CE")) # Get only collective efficacy terms
```
]
...
full_noquad_model <- 
  lm(CRIME_violent_2004_2006 ~ CE_hlm_2001 + CE_hlm_1995 + 
       ...
       BE_pr_bar_onstreet_block_2001 + BE_pr_liquor_onstreet_block_2001, 
     data = ccahs_block_analytical_unstd)
anova(full_noquad_model, full_model) |> tidy() 
```
]
...
```{r}
full_model |> tidy() |> filter(str_detect(term, "density_block"))
```
...
```{r}
ccahs_block_analytical_unstd |> pull(density_block) |> 
  range() |> round(3)
```

*Positive* effect of density at its min: $`r est_vec[1]` `r 2*est_vec[2]`*-0.035$ =  $`r est_vec[1] + (2*est_vec[2]*-0.035)`$<br>
*Negative* effect of density at its max: $`r est_vec[1]` `r 2*est_vec[2]`*0.236$ =  $`r est_vec[1] + (2*est_vec[2]*0.236)`$
...
You can learn about these in CaRM modules or textbooks

.footnote[
[1] I used a hierarchical negative binomial regression in this paper
]
...
* **Hierarchical models**
   ...
   * Repeated observations, e.g., person-years
      * **Panel models** are included here
...
One problem with my analysis is I don't observe *change* in the built environment—I only had one time point for those measures!

* There might be unmeasured differences between neighborhoods or blocks explaining my effects
* If I'd had at least two measures, I could make a more convincing test by *looking only at change*
