...
```{r}
shootings |> pull(race) |> table() |> sort()
```

--

Read this as, "get `shootings`, and then pull out the `race` column, and then make a `table()`, and then `sort()` it."
...
Community crime data based on those in yesterday's CRM lecture

```{r}
communities <- read_csv("https://clanfear.github.io/ioc_iqa/_data/communities.csv")
```
