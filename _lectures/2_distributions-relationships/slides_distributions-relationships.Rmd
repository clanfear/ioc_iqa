...
Maybe reorient this all so it matches the exact content from CRM? Maybe just export the CRM data?
...
Read this as, "take `communities`, and then pull out the `Incarceration` column, and then make a `table()`, and then `sort()` it."
...
```{r}
communities |>
  mutate(high_crime = `Crime Rate` > mean(`Crime Rate`)) |>
  head(4)
```
...
Let's look at tabulations first. They're most useful for summarizing categorical data.
...
* Recommended reading for next week
   ...
   * Kaplan, chapters 10 and 14 are a review of today's content
