...
sort(table(communities$Incarceration))

communities |> pull(Incarceration) |> table() |> sort()
...
communities %>% group_by(Area) %>% summarize(`Mean Crime Rate` = round(mean(`Crime Rate`), 2), 
