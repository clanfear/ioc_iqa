...
date: "5 Nov 2025<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%e %b %Y'))`"
output:
...
The group means that were different are now the same:
...
Subtracting the mean from every value just *shifts* the distribution to be centred over *zero*.

This *equalises* the means of each group!
...
*Controlling* for a categorical variable is equivalent to centering the other variables by their group categories
...
\tikzset{
    ...
    auto, font=\footnotesize, inner sep=0pt
    }
}
\tikz{
  \node (Y) at (0,5) {$Y$};
...
## An advanced example<sup>1</sup>

.footnote[
[1] i.e., you don't need to know this
]
...
&zwj;Reminder: Class next week **rescheduled** to Monday
...
* Read instructions carefully
   * No trick questions intended; everything was covered
...
* Due 11:59 PM Friday 14 November
