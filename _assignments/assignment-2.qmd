...
> Based on the plots from 2.4, we might expect there is an **interaction** between `control` and `opportunity`. That is, we think the effect of `control` on `robbery` might be different at different levels of `opportunity`. Take your linear model from 2.3—with the logged outcome and quadratic disadvantage—and add an interaction between `control` and `opportunity`.
